...
/* ===== NUEVO SUBRAYADO CON NORMALIZACIÓN (tildes incluidas) ===== */
...
private String highlightTerms(String text, List<String> terms) {
    ...
    for (String term : terms) {
        ...
        if (isSingleWord) {
            // palabra suelta: usar bordes de palabra en el texto normalizado
            ...
            // frase: buscar bloque normalizado
            int from = 0;
            while (true) {
    ...
    int pos = 0;
    for (R r : merged) {
        if (r.s > pos) out.append(escapeHtml(text.substring(pos, r.s)));
...
try {
    ...
    while (rs.next()) {
        ...
        // conteo de frase/palabra buscada
        if (hasText) {
            boolean matchText;
            if (q.contains(" ")) {
...
// normalización universal en JS (igual que en Java)
function norm(str) {
    if (!str) return "";
    return str
        .normalize("NFD")
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // conjunto de palabras activas para filtrar (clic en palabra)
    const activeKeys = new Set();

    function rowContainsAnyActive(row) {
        if (activeKeys.size === 0) return true; // sin filtros → todo
    ...
    function applyWordFilter() {
        rows.forEach(row => {
            const match = rowContainsAnyActive(row);
            row.dataset.wordFiltered = match ? "0" : "1";
        });
    ...
    wordBoxes.forEach(box => {
        ...
        // clic en la caja (palabra) → activar/desactivar filtro positivo
        ...
        // clic en el círculo hace lo mismo (no lógica distinta)
        if (circle) {
